...
export function NewsOverview({ status, onChanged }: { status: NewsStatus; onChanged: () => Promise<void> }) {
  ...
  const readinessNotes = [
    ...(!status.readiness.master_enabled ? [t("AI-news generation is disabled by the server.", "La generación de noticias de IA está desactivada en el servidor.")] : []),
    ...(status.readiness.openai === "missing" ? [t("Configure an OpenAI key to generate editions.", "Configura una clave de OpenAI para generar ediciones.")] : []),
    ...status.readiness.warnings.map(/** Translate optional-service codes and explain that they do not block generation. */ function explain(warning) {
      if (warning === "brave_not_configured") return t("Brave search is optional; official sources still work.", "La búsqueda de Brave es opcional; las fuentes oficiales siguen funcionando.");
      if (warning === "email_not_configured") return t("Email delivery is off; previews still work.", "El envío por correo está desactivado; las vistas previas siguen funcionando.");
      return warning;
    }),
  ];
  return <div className="news-overview">
    <section className="news-metric-grid">
      <article><Radar aria-hidden="true" /><span>{t("Readiness", "Preparación")}</span><strong>{status.readiness.ready ? t("Ready", "Lista") : t("Action needed", "Requiere acción")}</strong><small>{readinessNotes.join(" · ") || t("All required services are configured", "Todos los servicios necesarios están configurados")}</small></article>
    ...
    <section className="news-command-bar">
      <div><span className={`news-live-light ${status.schedule.enabled ? "on" : ""}`} /><div><strong>{status.schedule.enabled ? t("Weekly publishing enabled", "Publicación semanal activada") : t("Weekly publishing is off", "La publicación semanal está desactivada")}</strong><small>{status.schedule.activation_preview_run_id ? t("The full automatic pipeline has passed a test", "El proceso automático completo ha superado una prueba") : t("Test the full automatic pipeline before activation", "Prueba el proceso automático completo antes de activarlo")}</small></div></div>
      <div className="news-actions"><Button variant="outline" disabled={Boolean(status.active_run)} onClick={/** Generate a private manual preview without claim fact-checking. */ function preview() { void change(/** Request an ordinary current-window preview. */ function request() { return startNewsPreview(); }); }}><Play aria-hidden="true" />{t("Generate preview", "Generar vista previa")}</Button><Button variant="outline" disabled={Boolean(status.active_run)} onClick={/** Rehearse every automatic-publication gate before schedule activation. */ function activationTest() { void change(/** Request a full-path private activation preview. */ function request() { return startNewsPreview(null, true); }); }}>{t("Test weekly pipeline", "Probar proceso semanal")}</Button><Button variant="outline" disabled={Boolean(status.active_run)} onClick={/** Start the explicit fully checked immediate-publication workflow. */ function publishNow() { void change(runAndPublishNews); }}>{t("Run and publish", "Ejecutar y publicar")}</Button><Button disabled={!status.schedule.activation_preview_run_id} onClick={/** Toggle future scheduled starts after protected authorization. */ function schedule() { void change(/** Persist the opposite schedule state. */ function request() { return setNewsSchedule(!status.schedule.enabled); }); }}>{status.schedule.enabled ? t("Disable schedule", "Desactivar programación") : t("Enable schedule", "Activar programación")}</Button></div>
    </section>
    <p className="news-preview-policy">{t("Ordinary previews skip claim fact-checking and require administrator approval before publication. The weekly pipeline test and Run and publish retain the full evidence check.", "Las vistas previas normales omiten la verificación de afirmaciones y requieren aprobación administrativa para publicarse. La prueba semanal y Ejecutar y publicar mantienen la revisión completa de evidencias.")}</p>
