"use client";
import { Bell, BookOpenCheck, DatabaseZap, KeyRound, Newspaper, UsersRound } from "lucide-react";
...
const TABS: Array<{ id: NewsTab; icon: typeof Newspaper; en: string; es: string }> = [
  ...
  { id: "providers", icon: KeyRound, en: "Providers & models", es: "Proveedores y modelos" },
...
export function AiNewsPage() {
  ...
  return <div className="ai-news-workspace"><header className="news-hero"><div><span className="eyebrow">{t("AUTOMATED EDITORIAL OPERATIONS", "OPERACIONES EDITORIALES AUTOMATIZADAS")}</span><h1>{t("Weekly AI newsroom", "Sala de noticias de IA semanal")}</h1><p>{t("AI model releases become a bilingual roundup. Ordinary administrator previews skip claim fact-checking; automatic publication keeps the full evidence review.", "Los lanzamientos de modelos de IA se convierten en un resumen bilingüe. Las vistas previas normales omiten la verificación de afirmaciones; la publicación automática mantiene la revisión completa de evidencias.")}</p></div><div className="news-identity"><span className="seal">IA</span><div><strong>MABlog_IA</strong><small>{t("System-owned publisher", "Publicador propiedad del sistema")}</small></div></div></header><StepUpPanel /><nav className="news-tabs" aria-label={t("AI newsroom sections", "Secciones de la sala de IA")}>{TABS.map(/** Render one stable newsroom section control. */ function renderTab(item) { const Icon = item.icon; return <button key={item.id} aria-current={tab === item.id ? "page" : undefined} onClick={/** Show the selected newsroom section. */ function selectTab() { setTab(item.id); }}><Icon aria-hidden="true" />{t(item.en, item.es)}{item.id === "alerts" && data?.unread_alerts ? <span>{data.unread_alerts}</span> : null}</button>; })}</nav>{tab === "overview" ? data ? <NewsOverview status={data} onChanged={reload} /> : <Loading error={error} /> : null}{tab === "sources" ? <NewsSources /> : null}{tab === "editions" ? <NewsEditions /> : null}{tab === "alerts" ? <NewsAlerts /> : null}{tab === "providers" ? <NewsProviders /> : null}{tab === "administrators" ? <AdministratorManagement /> : null}<NewsRunPanel onChanged={reload} /></div>;
}
